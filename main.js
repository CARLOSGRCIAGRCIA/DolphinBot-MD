...
const { partidoCommand } = require("./commands/partida");
const { handlePartidoReaction } = require("./lib/reactionHandler");
const {
  autotypingCommand,
  isAutotypingEnabled,
...
global.channelLink = "https://whatsapp.com/channel/0029VbAfBzIKGGGKJWp5tT3L";
global.ytch = "Carlos G";
  ...
  try {
    ...
    switch (true) {
      ...
      case userMessage.startsWith(".n"):
        {
          const commandLength = userMessage.startsWith(".tag") ? 4 : 2;
      ...
      case userMessage.startsWith(".sora"):
        ...
        break;
      case userMessage.startsWith(".4v4"):
        {
          const args = ["4v4", ...userMessage.split(" ").slice(1)];
          await partidoCommand(sock, chatId, message, args);
        }
        commandExecuted = true;
        break;

      case userMessage.startsWith(".6v6"):
        {
          const args = ["6v6", ...userMessage.split(" ").slice(1)];
          await partidoCommand(sock, chatId, message, args);
        }
        commandExecuted = true;
        break;

      case userMessage.startsWith(".hexagonal"):
        {
          const args = ["hexagonal", ...userMessage.split(" ").slice(1)];
          await partidoCommand(sock, chatId, message, args);
        }
        commandExecuted = true;
        break;

      case userMessage.startsWith(".trilatero"):
        {
          const args = ["trilatero", ...userMessage.split(" ").slice(1)];
          await partidoCommand(sock, chatId, message, args);
        }
        commandExecuted = true;
        break;

      case userMessage.startsWith(".cuadrilatero"):
        {
          const args = ["cuadrilatero", ...userMessage.split(" ").slice(1)];
          await partidoCommand(sock, chatId, message, args);
        }
        commandExecuted = true;
        break;
...
async function handleReactionUpdate(sock, reaction) {
  try {
    if (!Array.isArray(reaction) || reaction.length === 0) {
      console.log("Formato de reacción inválido: no es array");
      return;
    }

    for (const react of reaction) {
      if (!react.reaction || !react.reaction.key) {
        console.log("Reacción sin datos válidos");
        continue;
      }

      const reactionData = {
        key: react.reaction.key,
        emoji: react.reaction.text,
        reactorKey: react.key,
      };

      if (!reactionData.emoji) {
        console.log("No se encontró emoji en la reacción");
        continue;
      }

      await handlePartidoReaction(sock, reactionData);
    }
  } catch (error) {
    console.error("Error en handleReactionUpdate:", error);
  }
}
