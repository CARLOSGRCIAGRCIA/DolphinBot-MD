  ...
  try {
    ...
    XeonBotInc.ev.on("connection.update", async (s) => {
      ...
      if (connection == "open") {
        ...
        console.log(
          chalk.magenta(`${global.themeemoji || "•"} GITHUB: CARLOSGRCIAGRCIA`)
        );
        console.log(
          chalk.magenta(`${global.themeemoji || "•"} WA NUMBER: ${owner}`)
        );
        console.log(
          chalk.magenta(`${global.themeemoji || "•"} CREDIT: Carlos G`)
        );
        console.log(
          chalk.green(
    ...
    XeonBotInc.ev.on("messages.reaction", async (reaction) => {
      try {
        ...
        // );
        await handleReactionUpdate(XeonBotInc, reaction);
      } catch (error) {
        console.error("Error in messages.reaction handler:", error);
      }
    });
    // XeonBotInc.ev.on("messages.update", (update) => {
    //   console.log("Messages update:", JSON.stringify(update, null, 2));
    // });
