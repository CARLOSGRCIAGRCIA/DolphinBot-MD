...
async function convertBufferToStickerWebp(inputBuffer, isAnimated, cropSquare) {
  ...
  const json = {
    ...
    "sticker-pack-name": settings.packname || "ThunderBot",
    emojis: ["📸"],
  };
  ...
  if (finalBuffer.length > 900 * 1024) {
    try {
      ...
      const vfSmall = cropSquare
        ...
        : `scale=320:320:force_original_aspect_ratio=decrease,pad=320:320:(ow-iw)/2:(oh-ih)/2:color=#00000000${
            isAnimated ? ",fps=8" : ""
          }`;
      const cmdSmall = `ffmpeg -y -i "${tempInput}" ${
        isAnimated ? "-t 2" : ""
      } -vf "${vfSmall}" -c:v libwebp -preset default -loop 0 -vsync 0 -pix_fmt yuva420p -quality ${
        isAnimated ? 28 : 65
      } -compression_level 6 -b:v 80k -max_muxing_queue_size 1024 "${tempOutput3}"`;
      await new Promise((resolve, reject) => {
...
async function forceMiniSticker(inputBuffer, isVideo, cropSquare) {
  ...
  const vf = cropSquare
    ...
    : `scale=256:256:force_original_aspect_ratio=decrease,pad=256:256:(ow-iw)/2:(oh-ih)/2:color=#00000000${
        isVideo ? ",fps=6" : ""
      }`;

  const cmd = `ffmpeg -y -i "${tempInput}" ${
    isVideo ? "-t 2" : ""
  } -vf "${vf}" -c:v libwebp -preset default -loop 0 -pix_fmt yuva420p -quality 25 -compression_level 6 -b:v 60k "${tempOutput}"`;
